...
class DeepONet(nn.Module):
    ...
    def forward(
        ...
        if branch_out.dim() == 2 and trunk_out.dim() == 2:
            output = torch.einsum("bi,bi->b", branch_out, trunk_out)
            output = output.unsqueeze(-1)
        elif branch_out.dim() == 2 and trunk_out.dim() == 3:
            output = torch.einsum("bd,bnd->bn", branch_out, trunk_out)
            output = output.unsqueeze(-1)
        else:
...
class DeepONetConfig:
    ...
    @classmethod
    def from_dict(cls, config_dict: dict) -> "DeepONetConfig":
