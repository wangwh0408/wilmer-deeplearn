...
class ModelManager:
    ...
    @classmethod
    def load_model(cls, model_path: Optional[str] = None) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            modes = checkpoint.get('modes', settings.DEFAULT_MODES)
            width = checkpoint.get('width', settings.DEFAULT_WIDTH)
            in_channels = checkpoint.get('config', {}).get('in_channels', settings.DEFAULT_IN_CHANNELS)
            out_channels = checkpoint.get('config', {}).get('out_channels', settings.DEFAULT_OUT_CHANNELS)
            resolution = checkpoint.get('resolution', settings.DEFAULT_RESOLUTION)

            logger.info(
                f"Model config from checkpoint: modes={modes}, width={width}, "
            ...
            return True, "Model loaded successfully", cls._model_info
    ...
    def predict(
        ...
        source_tensor = torch.tensor(source, dtype=torch.float32).to(settings.DEVICE)

        with torch.no_grad():
            output = cls._model(source_tensor)
...
async def lifespan(app):
    if os.path.exists(settings.MODEL_PATH):
        logger.info(f"Auto-loading model from: {settings.MODEL_PATH}")
        ...
        logger.info(
            ...
            f"Use POST /api/model/load to load a model."
        )
